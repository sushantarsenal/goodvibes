...
const getSelectStyleSetting = (css, isSearchable, hasError, isActive, disabled) => ({
  ...
  control: styles => ({
    ...
    border      : borderColor({ isActive, hasError }),
    borderRadius: 6,
    minHeight   : 38,
    height      : 44,
    padding     : '0 12px',
    ...
    <InputWrapper css={inputCss}>
      ...
      <SelectInputWrapper>
        <Select
          ...
          options={options}
          onKeyDown={event => {
            if(allowNewOptions && event.keyCode === 13 && event.target.value) {
              input.onChange({ id: null, value: event.target.value })
          ...
          noOptionsMessage={() => allowNewOptions ? null : 'No Options'}
